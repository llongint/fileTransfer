/*************************************************************************
    ...
    > Created Time: Thu 23 Aug 2018 08:41:21 PM CST
    > function: RSA公钥加密相关函数,参考 https://github.com/pantaloons/RSA
 ************************************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
...
/** 
 * @brief  生成一个随机素数.这个函数会生成一个给定长度的10进制随机数,
 *          转化为大数后向上搜索第一个合适的素数
 * @note   
 * @param  numDigits: 数字的长度
 * @param  result: 生成的结果
 * @retval None
 */
void randPrime(int numDigits, bignum* result) {
	...
	int i;
	string[0] = (rand() % 9) + '1';                 /* 最前面的数字不能是0 */
	string[numDigits - 1] = (rand() % 5) * 2 + '1'; /* 最后一个时奇数 */
	for(i = 1; i < numDigits - 1; i++)
        string[i] = (rand() % 10) + '0';            /* 填充中间的数字 */
	string[numDigits] = '\0';
	bignum_fromstring(result, string);
	while(1) {
		if(probablePrime(result, ACCURACY)){        /* 判断是否是素数 */
			free(string);
			return;
		}
 ...
 */
/** 
 * @brief  为RSA公钥算法选择一个随机的公钥指数,指数会比系数小
 * @note   
 * @param  phi: 
 * @param  n: 
 * @param  result: 
...
int create_key(const char const *public_key,const char const *private_key)
{
    ...
    /* 4.取一个小的与r互素的数e,与 n 组成公钥 */
    randExponent(phi, EXPONENT_MAX, e);

    /* 5.计算出e关于模r的数论倒数d,与n组成私钥 */
